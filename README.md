...
3.  Install dependencies:
    ...
    npm install
    ```
4.  Start the Node.js Express server (requires environment variables):
    ```bash
    JWT_SECRET=your-secret DATABASE_URL=postgresql://localhost/gilmore_db npm start
    ```
...
#### Unit Tests
Verifies the logic in `logic.js` using Node.js native test runner. Requires the `JWT_SECRET` environment variable.
