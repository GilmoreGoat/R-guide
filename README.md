...
3.  Install dependencies:
    ...
    npm install
    ```
4.  Start the Node.js Express server (requires a PostgreSQL connection string):
    ```bash
...
1. Create the HTML file in the `modules/` directory.
2. Link it in `index.html`'s navigation.
3. Add it to the `rPages` array in `tests/verify_pages_load.spec.js` to ensure Playwright verification.
4. (Optional) Configure its R packages in `PAGE_PACKAGES` within `js/logic.js`.
