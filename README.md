...
3.  Install dependencies:
    ...
    npm install
    ```
4.  Start the Node.js Express server with the required environment variables:
    ```bash
...
#### Unit Tests
Verifies the logic in `logic.js` using Node.js native test runner. Running tests also requires the `JWT_SECRET` environment variable to be set.
