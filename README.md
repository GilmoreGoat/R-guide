...
3.  Install dependencies:
    ...
    npm install
    ```
4.  Start the Node.js Express server (requires `JWT_SECRET` and `DATABASE_URL` for PostgreSQL):
    ```bash
...
#### End-to-End Tests
Verifies WebR loading and page interactions using Playwright. Requires `JWT_SECRET` and a running local server with database connection.
