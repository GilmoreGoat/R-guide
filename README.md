...
3.  Install dependencies:
    ...
    npm install
    ```
4.  Start the Node.js Express server. You must set the `JWT_SECRET` environment variable (the server will exit if it is missing):
    ```bash
...
Both Unit and End-to-End Tests require the `JWT_SECRET` environment variable to be set.

#### Unit Tests
Verifies the logic in `logic.js` and middleware using the Node.js native test runner. The `test` script in `package.json` provides a default `test-secret` for convenience.
...
#### End-to-End Tests
Verifies WebR loading and page interactions using Playwright. You may need to install Playwright browsers first:

```bash
npx playwright install chromium
```

Run the tests with a dummy secret:
