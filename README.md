...
3.  Install dependencies:
    ...
    npm install
    ```
4.  Start the Node.js Express server (requires `DATABASE_URL` and `JWT_SECRET` environment variables):
    ```bash
...
#### Unit Tests
Verifies the logic in `logic.js` using Node.js native test runner. Note that `JWT_SECRET` is required.
...
#### Visual Verification
Verifies UI visually and takes screenshots using Python Playwright. Requires the server to be running locally on port 8000.
