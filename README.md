...
4.  Start the Node.js Express server:
    ...
    ```
    *(Note: `JWT_SECRET` must be set, or the application will exit).*
...
1. Creating its HTML file in `modules/`.
2. Linking it in `index.html`'s navigation.
3. Adding it to the `rPages` array in `tests/verify_pages_load.spec.js` to ensure Playwright verification.
4. (Optional) Configuring its R packages in `PAGE_PACKAGES` within `js/logic.js`.

### Testing

#### Unit Tests
Verifies logic in utility and middleware functions using Node.js native test runner. The test script provides a dummy `test-secret` for local execution.
...
#### End-to-End Tests
Verifies WebR loading and page interactions using Playwright. Requires a running local Node server with a database connection.
...
We use conventional PR titles:
- 🧹 **Code Health:** Title format `🧹 [description]`. Description must include What, Why, Verification, and Result.
- 🧪 **Testing:** Title format `🧪 [description]`. Description must detail gap addressed, coverage scenarios, and result.
- ⚡ **Performance:** Title format `⚡ [description]`. Description must detail optimization, rationale, and measured improvement.
