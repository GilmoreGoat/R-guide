...
4.  Set up environment variables (optional but recommended for production):
    ...
    - `DATABASE_URL`: Connection string for PostgreSQL database. If not set, it defaults to `postgresql://localhost/gilmore_db`.
5.  Start the Node.js Express server:
    ```bash
    DATABASE_URL=postgresql://localhost/gilmore_db JWT_SECRET=your-secret-key npm start
    ```
    Or simply `npm start` to use default local settings.
