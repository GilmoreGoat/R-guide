...
- PostgreSQL database (for storing user progress and authentication data).
...
4.  Set up the required environment variables:
    - `DATABASE_URL`: Connection string for your PostgreSQL database (e.g., `postgresql://localhost/gilmore_db`).
    - `JWT_SECRET`: A secret key used to sign JSON Web Tokens.
5.  Start the Node.js Express server:
    ```bash
    DATABASE_URL=postgresql://localhost/gilmore_db JWT_SECRET=your-secret-key npm start
    ```
...
#### Unit Tests
Verifies the logic in `logic.js` using Node.js native test runner. Note that `JWT_SECRET` is required to run tests.
...
#### End-to-End Tests
Verifies WebR loading and page interactions using Playwright. The local server must be running, and the `JWT_SECRET` environment variable must be set.
