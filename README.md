...
3.  Install dependencies:
    ...
    npm install
    ```
4.  Start the Node.js Express server (requires `JWT_SECRET` and `DATABASE_URL`):
    ```bash
...
- `add_download_buttons.py`: Python script to add download buttons to HTML pages.
- `create_challenges.py`: Python script to generate R Markdown challenge templates.
- `create_tier4_projects.py`: Python script to dynamically generate zip file challenges.
- `fix_fouc.py`: Python script to inject a script tag to prevent Flash of Unstyled Content (FOUC) when loading the dark theme.
