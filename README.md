...
Project testing is split into Unit Tests and End-to-End Tests. Both require the `JWT_SECRET` environment variable. The `test` script in `package.json` is configured to provide a dummy secret (`test-secret`) for local unit testing.
...
#### End-to-End Tests
Verifies WebR loading and page interactions using Playwright. E2E tests additionally require the local Node server running with a database connection and may require installing Playwright browsers.

```bash
# Install browsers if needed
npx playwright install chromium

# Run the tests with a dummy secret (if not set in env)
