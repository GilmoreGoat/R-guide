...
- **Dark Mode**: Support for system-based and user-toggled dark themes via `localStorage` and `data-theme` attributes.
...
3.  Install dependencies:
    ...
    npm install
    ```
4.  Start the Node.js Express server by specifying the required `DATABASE_URL` and `JWT_SECRET` environment variables. For example, for local development:
    ```bash
...
Both Unit and End-to-End Tests require the `JWT_SECRET` environment variable to be explicitly provided.

#### Unit Tests
Verifies the logic in `logic.js` using Node.js native test runner. To run with coverage, use the `--experimental-test-coverage` flag.
...
#### End-to-End Tests
Verifies WebR loading and page interactions using Playwright. Requires the local Node server to be running in the background with a database connection.
