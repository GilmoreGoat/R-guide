...
- `add_download_buttons.py`: Python script to add download buttons to the project's HTML pages.
- `create_challenges.py`: Python script to generate challenge files as `.Rmd` files inside `.zip` archives.
- `create_tier4_projects.py`: Python script to dynamically generate zip file challenges.
- `fix_fouc.py`: Python script to inject a synchronous `<script>` into HTML files to apply the dark theme data attribute, preventing a Flash of Unstyled Content (FOUC).
